...
// usfmBook returns the 3-character USFM book code for a canonical book
// name (case-insensitive). The empty string indicates an unknown book.
// Callers should already have validated the name via canon.LookupBook
// or canon.ValidateQuery before reaching this point.
func usfmBook(name string) string {
	return usfmByName[strings.ToLower(strings.TrimSpace(name))]
}
