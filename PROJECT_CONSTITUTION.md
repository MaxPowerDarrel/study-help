...
One exception to the web-only rule exists: a native iOS app, first permitted **solely to host a home-screen widget** that surfaces the day's reading assignment (widgets cannot be built with web technology), with reading delegated to the same hosted SPA in a web view. *(Amended 2026-06-05 — see [`specs/native-ios.md`](./specs/native-ios.md). The widget consumes only daily reading **references**, never scripture text, and the backend is unchanged.)* That app may additionally render passages **natively**, as a second consumer of the same JSON API the web client uses. The web app remains the **canonical** reading implementation — reading features are specified and shipped web-first, and the embedded-SPA web view remains available inside the app as a fallback while the native reader matures. *(Amended 2026-06-09 — see [`specs/native-reader.md`](./specs/native-reader.md). The native reader changes no backend behavior, caches no scripture text on device, and is removable in a single PR.)*
