...
- `internal/dailyreader/` — embedded reading-plan parsers (Bible-in-One-Year markdown, 2026 Hope plan markdown, generated Study Plan markdown). Returns the day's passages per plan; the SPA fetches each pill against the active translation. Cell parsing shared across plans lives in `passage_cell.go`. The Study Plan table (`study-plan.md`, per `specs/study-plan.md`: whole OT across the year, whole NT each quarter, a daily Psalm) is generated — run `go generate ./internal/dailyreader/...` after changing `gen/main.go`; never hand-edit the table.
