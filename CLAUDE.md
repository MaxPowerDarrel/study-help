...
Reader feature shipped per `specs/passage-reader.md`. The Go server proxies ESV passage requests, exposes a Prometheus counter on a localhost-only metrics port, and serves the React SPA from an embedded Vite build. Accounts shipped per `specs/accounts.md`: server (signup / signin / signout / me, cookie sessions, per-IP + per-account rate limiting) + client (top-right chip, slide-in sign-in / create-account panel, sign-out menu). Highlights shipped per `specs/highlights.md`: server (`internal/highlights/` + three endpoints behind `RequireUser`) and client (`web/src/highlights/` floating toolbar, range-based selection → `<mark>` overlay, `web/src/platform/SelectionAdapter`); ESV client sends `include-verse-anchors=true` unconditionally so the client can locate selections by verse + offset. Daily annotations shipped per `specs/daily-annotations.md`: multi-chapter daily pills expand to per-chapter `PassageView`s with their own `articleRef`s, per-chapter spinners, and a daily-header translation picker. The daily-tab state machine, per-chapter ref map, and panel render live under `web/src/daily/` (`useDailyTab.ts` + `DailyPanel.tsx`). The Notes feature was removed on 2026-05-07 (privacy concern over storing free-form user prose); see `specs/notes.md` for the historical record and `00006_remove_notes.sql` for the table drop.

**Layout:**

- `main.go` — wires config → DB → auth service → public server + private metrics server, with graceful SIGINT/SIGTERM shutdown.
- `internal/config/` — `Config` struct loaded from env vars: `ADDR` (default `:8080`), `DATABASE_URL` (default `./sqlite.db`), `SESSION_SECRET` (required), `ESV_API_KEY` (required), `YOUVERSION_APP_KEY` (required, used by the NIV provider), `ENV` (default `prod`; set to `dev` to drop the `Secure` cookie flag for plain-HTTP localhost).
- `internal/db/` — opens SQLite (WAL, foreign keys, 5s busy timeout) via `modernc.org/sqlite` (pure-Go, no CGO) and runs goose migrations from the embedded `migrations/` directory. Migrations: `00001_users.sql`, `00002_sessions.sql`, `00003_highlights.sql`, `00004_notes.sql` (historical; the table is dropped by `00006_remove_notes.sql`), `00005_translation.sql`, `00006_remove_notes.sql`. Also exposes `db.TX`, the small `Exec/Query` interface used by query helpers in `internal/auth` and `internal/highlights` so they accept either `*sql.DB` or `*sql.Tx`.
...
- `internal/httpx/` — small HTTP helpers shared across handler packages: `WriteJSON`, `BadRequest`, `ParsePassageQuery` (validates `?book=&chapter=` against the canon), and `ResolveTranslation` (supplied → fallback → registry check). Used by `internal/highlights`.
...
- `internal/highlights/` — per-user range-based annotations against the ESV canon. Exposes `GET /api/highlights?book&chapter`, `POST /api/highlights`, `DELETE /api/highlights/{id}`, all behind `auth.RequireUser`. Half-open character ranges over `(book, chapter, start_verse, start_offset, end_verse, end_offset)`; overlap check in the Go handler before insert; cross-user delete returns 404 (no ID enumeration). Reuses `canon.LookupBook` for canon validation.
- `internal/esv/` — server-side ESV API client (`api.esv.org/v3/passage/html/`) plus the `Provider` adapter that exposes it as a `scripture.Provider`. The ESV API key never reaches the browser (constitution §4).
...
- `web/` — React SPA (Vite + TypeScript). Picker pane, reading surface with formatting toggles, light/dark/system theme, accounts UI. Component styles live in `*.module.css` (CSS Modules); design tokens in `web/src/styles/tokens.css` (incl. `--color-highlight`); ESV-rendered HTML styled globally in `web/src/styles/passage.css` (incl. `mark.highlight`). Theme persistence at `web/src/theme.ts`, wired through the localStorage-backed `ToggleStore` at `web/src/platform/ToggleStore.ts` (consumed via `useSyncExternalStore` so multiple subscribers stay in sync); `web/src/platform/SelectionAdapter.ts` is the matching abstraction for the browser Selection API (the two are the §4 platform-abstraction layer). `web/src/platform/useResource.ts` is a generic async-cache hook (fetch-on-deps, refetch-on-mutation, `AbortController`-based cancellation) used by `useHighlights`. Auth client lives in `web/src/auth/`: `api.ts` (discriminated-union fetchers for the four `/api/auth/*` endpoints), `useUser.ts` (cold-load `/me` hydration + signin/signup/signout actions), `AuthChip.tsx` (top-right chip + sign-out menu), `AuthPanel.tsx` (slide-in panel with sign-in default + create-account toggle). Highlights client lives in `web/src/highlights/`: `api.ts` (discriminated-union fetchers for the three `/api/highlights*` endpoints), `useHighlights.ts` (per-passage cache; re-fetches on book/chapter change and after every successful mutation), `parseSelection.ts` + `applyHighlights.ts` (Range ↔ verse-offset tuples, `<mark data-highlight-id="...">` overlay), `HighlightToolbar.tsx` (floating toolbar with selection / existing / guest / error modes), `PassageView.tsx` (extracted from `App.tsx` to host article + selection wiring; `articleRef` is lifted to `App.tsx`). Daily-tab logic lives under `web/src/daily/`: `useDailyTab.ts` owns the load/reset effects, fetchId race guard, and per-chapter article-ref map; `DailyPanel.tsx` renders the per-pill stack of `PassageView`s plus the daily header. `App.tsx` keeps cross-tab concerns only (read-tab state, auth/settings panes). Tests via vitest + @testing-library/react: `npm test` in `web/`.
