...
| iOS client | **Swift / SwiftUI** (`ios/`), project generated by **[XcodeGen](https://github.com/yonaskolb/XcodeGen)** from `ios/project.yml` | Native app + daily-reading widget (`specs/native-ios.md`) and the native reading surface (`specs/native-reader.md`). TestFlight distribution. |
| iOS HTML parsing | **[SwiftSoup](https://github.com/scinfu/SwiftSoup)** (SwiftPM, app target only) | The native reader parses provider HTML into `AttributedString`. Publisher HTML isn't well-formed XML (`XMLParser` aborts) and `NSAttributedString(html:)` is WebKit-backed and discards the `class` attributes that carry verse/woc/cross-ref semantics. Decided in `specs/native-reader.md`. |
